...
class TourRetrievalPipeline:
    ...
    def get_tour_response(self, query, top_k=3, user_id="default_user"):
        ...
        if intent == "out_of_scope":
            #có thể gọi gemini để hỏi tự nhiên hơn
        ...
        miss_info = []
        if not info["location"]:
            miss_info.append("Điểm đến (ví dụ: Đà Lạt, Hà Nội, Phú Quốc)")
        if not info["time"]:
            miss_info.append("Thời gian khởi hành (ví dụ: tháng 12, 2 tuần tới)")
        if not info["price"]:
            miss_info.append("Giá (ví dụ: dưới 5 triệu, khoảng 2 triệu)")
        ...
        if miss_info:
            prompt = (
                ...
                f"Ví dụ: 'Dạ, em đã ghi nhận thông tin quý khách muốn tìm tour đến Đà Lạt. Xin quý khách vui lòng cho em biết thêm thông tin như:'"
